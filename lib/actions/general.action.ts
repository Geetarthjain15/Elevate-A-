...
export async function createFeedback(params: CreateFeedbackParams) {
  ...
  try {
    ...
    if (!isFirebaseAdminConfigured) {
      return { success: true, feedbackId: "demo-feedback-001" };
    }
...
export async function getLatestInterviews(
  ...
  // To avoid requiring complex composite indexes in Firebase, we fetch the latest and filter in memory
...
export async function getInterviewsByUserId(
  ...
  // Fetch by userId and then sort in memory to avoid needing a composite index on (userId, createdAt)
  const interviews = await db
    .collection("interviews")
    .where("userId", "==", userId)
    .get();
